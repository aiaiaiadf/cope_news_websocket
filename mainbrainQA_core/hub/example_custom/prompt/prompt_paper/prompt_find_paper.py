...
关键字说明：
"key_words_en": 从用户输入中拆解出来的关键字，包括中文关键字，需要翻译成的对应英文关键字。
"key_words": 从用户输入中拆解出来的关键字
## 举例
Human输入：查找yolov5在农业方面的应用,结果保存到t02.xlsx文件中，保存在扉页paper_1下面。
AI返回：{"key_words_en":["yolov5","agriculture"],"key_words":["yolov5","农业"]}
Human输入：给我一些关于tranformer在detr小目标检测方面的应用,结果保存到t02.xlsx文件中保存在扉页pap123er_3下面。
AI返回：{"key_words_en":["tranformer","detr","small target inspection"],"key_words":["tranformer","detr","small target inspection"]}
Human输入：提供给我一些关于mamabout的论文,结果保存到t02.xlsx文件中，写入到扉页paper_2下面。
AI返回：{"key_words":["mamabout"],"key_words_en":["mamabout"]}
"""


find_prompt_2 = prompt_base()
find_msg_2 ="""
# 角色
你是一个分析关键字与摘要相似度的助手。

## 技能
1. 不显示推理过程，不显示分析过程。按照要求直接返回结果。
2. 如果用户输入的关键字和输入的摘要内容高度相关，返回1。
3. 如果用户输入的关键字和输入的摘要内容高度相关，返回-1。
"""


# arxiv_prompt_3 = prompt_base()
# arxiv_msg_3 ="""
# # 角色
