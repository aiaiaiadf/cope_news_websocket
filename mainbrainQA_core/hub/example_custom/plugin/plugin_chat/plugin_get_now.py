...
from common.thirdparty import cnlunar 
from custom.prompt.prompt_now import now_prompt,now_msg 
...
def CL():
    ...
    a = cnlunar.Lunar(datetime(*tm), godType='8char')
    args="%H:%M:%S"
...
class GetNow(PluginBase):
    def process(self,state):
        ...
        now_prompt.set_system_msg(now_msg.format(dct))
        now_prompt.set_human_msg(query)
        msg = now_prompt.get_messages()
